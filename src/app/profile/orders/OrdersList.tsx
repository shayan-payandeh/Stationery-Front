...
async function OrdersList() {
  const { data, error } = await getMyOrderServerComponent();
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center py-6 text-warning-500">
  ...
  const renderStatus = (status: string) =>
    status === "completed" ? (
      <span className="rounded-md bg-[#cdf2d5] p-[6px]">تکمیل شده</span>
    ) : (
      <span className="rounded-md bg-red-100 p-[6px]">درحال انجام</span>
    );
      ...
      <div className="flex flex-col overflow-x-auto">
        <table className="relative w-full min-w-[600px] overflow-hidden">
          ...
          >
            {orders.map((order) => (
              <tr key={order._id}>
                <td>#{toPersianNumbers(order._id.slice(0, 8))}</td>
                ...
                <td className="text-[.7rem]">{renderStatus(order.status)}</td>
                <td>
